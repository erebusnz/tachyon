...
Work through these in order on the schematic sheet containing U6, U2, U7:

- [ ] Wire U6 pin 5 (SYNC) to net `DAC-SPI-CS`; wire same net to STM32 PB1
- [ ] Wire U6 pin 6 (SCLK) to net `DAC-SPI-SCLK` (shared with OLED on PB13)
- [ ] Wire U6 pin 7 (DIN) to net `DAC-SPI-MOSI` (shared with OLED on PB15)
- [ ] Tie U6 pin 1 (VDD) to `+3V3_PREC`, place C_VDD1 (10 µF) + C_VDD2 (100 nF) at pin 1
- [ ] Tie U6 pin 2 (VREF) to `VREF_2V5`, place C_VREF (10 µF) at pin 2
- [ ] Tie U6 pin 8 (GND) to GND with a dedicated via to L2
- [ ] Wire U6 pin 4 (VOUTA) → U7 pin 3 (+IN A)
- [ ] Wire U6 pin 3 (VOUTB) → U7 pin 5 (+IN B)
- [ ] Tie U2 pin 1 (VIN) to `+5V` with C_REF_IN (100 nF) at pin 1
- [ ] Place C_NR (100 nF) from U2 pin 2 (TRIM/NR) to GND
- [ ] Leave U2 pin 3 (TEMP) and pin 7 (NC) floating
- [ ] Tie U2 pins 4 and 5 (GND) to GND with dedicated vias
- [ ] Tie U2 pin 6 (VOUT) to net `VREF_2V5` with C_REF_OUT (10 µF) at pin 6
- [ ] Tie U2 pin 8 (EN) to GND
- [ ] Tie U7 pin 8 (V+) to `+12V`, place C_VP1 (10 µF) + C_VP2 (100 nF) at pin 8
- [ ] Tie U7 pin 4 (V−) to `−12V`, place C_VN1 (10 µF) + C_VN2 (100 nF) at pin 4
- [ ] Place R1 (30 kΩ 0.1 %) between U7 pin 1 (OUT A) and pin 2 (−IN A)
- [ ] Place R2 (10 kΩ 0.1 %) between U7 pin 2 (−IN A) and GND
- [ ] Place R3 (30 kΩ 0.1 %) between U7 pin 7 (OUT B) and pin 6 (−IN B)
- [ ] Place R4 (10 kΩ 0.1 %) between U7 pin 6 (−IN B) and GND
- [ ] Place R_PROT_A (1 kΩ 1 %, AC0805FR-7W1KL) between U7 pin 1 (OUT A) and CV jack A tip — R1 feedback tap must be on the U7 pin 1 side of R_PROT_A
- [ ] Place R_PROT_B (1 kΩ 1 %, AC0805FR-7W1KL) between U7 pin 7 (OUT B) and CV jack B tip — R3 feedback tap must be on the U7 pin 7 side of R_PROT_B
- [ ] Tie CV jack A and B sleeves to GND
- [ ] Confirm U6, U2, U7 each have local GND vias dropping directly to L2 within < 1 mm of their GND pins
- [ ] Confirm `VREF_2V5` trace from U2 pin 6 to U6 pin 2 is ≤ 20 mm and routes over a clean L2 region
- [ ] Confirm `DAC-SPI-CS` idles HIGH in the firmware SPI init (PB1 GPIO output, initial state HIGH)
- [ ] Update `hardware-design-plan.md` pin-budget table to confirm PB1 (DAC CS) is reserved (PB13 / PB15 already reserved by OLED)
