...
int main(void)
{
  ...
  MX_TIM4_Init();
  MX_I2S3_Init();
  /* SD card detect is NC switch: LOW=no card, HIGH=card inserted */
...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  sConfig.Rank = 1;
  /* 56 cycles per cv-input.md §5 (was 3 — too short for the ~1 kOhm source;
   * also gives the polled scan loop ~3.2 us/conversion of margin to read each
   * rank before the next completes). Reused for ranks 2 and 3 below. */
...
static void MX_TIM2_Init(void)
{
  ...
  sConfigIC.ICFilter = 4;  /* fDTS/2, N=6 debounce (clock-input.md §5) — rejects
                            * sub-us edge ringing on the CLK-IN capture */
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, OLED_RES_Pin|OLED_DC_Pin|MUTE_N_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, DAC_SPI_CS_Pin|OLED_SPI_CS_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pins : DAC_SPI_CS_Pin PB2 */
  GPIO_InitStruct.Pin = DAC_SPI_CS_Pin|GPIO_PIN_2;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  /*Configure GPIO pin : OLED_SPI_CS_Pin */
  ...
  /*Configure GPIO pin : MUTE_N_Pin */
  GPIO_InitStruct.Pin = MUTE_N_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(MUTE_N_GPIO_Port, &GPIO_InitStruct);
