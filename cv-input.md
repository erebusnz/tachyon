...
- **U?? — OPA1642AIDR** — dual JFET op-amp, one channel per input,
...
|---|---|---|
| Number of inputs | 2 | `CV_IN_1`, `CV_IN_2` (modulation; not V/oct pitch) |
| Nominal input range | ±5 V | Doepfer A-100 standard modulation CV range; maps to full ADC scale |
| Absolute max input | ±12 V (clipped) | Over-range is passively clipped by op-amp rails and the BAT54S at the ADC node; the stage is not damaged |
...
| Attenuation (voltage gain) | −0.332 | 10 V p-p input → 3.3 V p-p ADC swing; full 12-bit resolution across ±5 V |
| DC bias source | REF5025 2.5 V → resistor divider → non-inverting pin | Shares the precision reference already on board |
...
| Input protection | Passive — series R_in 100 kΩ limits fault current into op-amp; BAT54S clamp on ADC node | No diodes on jack tip; op-amp handles the heavy lifting |
...
V_bias comes from the REF5025 2.5 V via a 10.2 kΩ / 10.0 kΩ divider:

    V_bias = 2.500 · 10.0 / (10.2 + 10.0) = 1.2376 V

    V_out = 1.2376 · 1.330 − 0.330 · V_in
          = 1.6460 − 0.330 · V_in

| V_in | V_out | ADC code (12-bit, 3.3 V ref) |
|---|---|---|
| +10.00 V (over-range) | −1.65 V → clamped to 0 V by BAT54S | 0 (saturated) |
| +5.00 V  | −0.004 V → clamped to 0 V | 0 |
| +2.50 V  | 0.821 V | ~1019 |
| 0.00 V   | 1.646 V | ~2043 |
| −2.50 V  | 2.471 V | ~3068 |
| −5.00 V  | 3.296 V | ~4091 |
| −10.00 V (over-range) | +4.95 V → clamped to 3.3 V by BAT54S | 4095 (saturated) |

The nominal ±5 V range maps to very nearly the full 12-bit ADC span,
with ~4 mV of zero-offset error from the 10.2 k / 10.0 k divider
(1 LSB at the ADC, ~12 mV referred to the input) that calibrates out
in firmware. Over-range input up to ±12 V is passively clipped by
the BAT54S at the ADC node through R_ao — the stage is not damaged
...
**Firmware note:** the stage is inverting, so `V_in = (1.646 − V_adc)
...
|---|---|---|---|---|
| R_in1, R_in2 | 100 kΩ 1 % | 0805 | Vishay CRCW0805100KFKEA (LCSC TODO) | Doepfer A-100 standard CV input impedance; also the fault-current limiter into the op-amp |
| R_f1, R_f2 | 33.0 kΩ 1 % | 0805 | Vishay CRCW080533K0FKEA (LCSC TODO) | Feedback; together with R_in sets the −0.330 attenuation |
| R_ao1, R_ao2 | 1.0 kΩ 1 % | 0805 | Existing BOM 1 k line (or C17513) | Series resistor into ADC pin; part of anti-alias RC and isolates op-amp from C_ao capacitive load |
| C_ao1, C_ao2 | 100 nF X7R | 0805 | Existing BOM 100 nF line | Anti-alias cap to GND; fc ≈ 1.6 kHz with R_ao — matched to CV modulation bandwidth |
| D_clamp1, D_clamp2 | BAT54S | SOT-23 | BAT54S (LCSC C7465) | Dual Schottky; clamps ADC pin to GND and +3V3 rails |
...
| Ref | Value | Package | MPN / LCSC | Notes |
|---|---|---|---|---|
| R_b1 | 10.2 kΩ 0.1 % | 0805 | Panasonic ERA-6AEB102V2 (LCSC TODO) | Divider top, tied to REF5025 2.5 V |
| R_b2 | 10.0 kΩ 0.1 % | 0805 | Panasonic ERA-6AEB103V (LCSC TODO) | Divider bottom, tied to GND |
| C_b  | 10 nF X7R | 0805 | Existing BOM 10 nF line | Noise filter at the divider tap; one cap for both channels |

0.1 % on the divider keeps V_bias within ~1 LSB of ideal; 1 % parts
would give ~12 mV of offset error referred to the input, which
calibrates out in firmware but eats into the over-range headroom.
If 0.1 % parts are hard to source, use 1 % and store the per-channel
zero-offset in the calibration block.

**Op-amp:** one dual OPA1642AIDR package covers both channels. Call
it **U11** in the schematic (next free designator after U10). Rails
...
**BAT54S** dual Schottky at the ADC node clamps any overshoot to the
digital 3V3 rail (top) and GND (bottom):

- Worst case: op-amp output pinned at +12 V → V across R_ao ≈ 8.7 V
  → 8.7 mA through R_ao into the upper BAT54S diode. BAT54S handles
...
even without BAT54S the pin would probably survive; BAT54S is cheap
...
|---|---|---|---|
| `CV_IN_1` | **PA0** | ADC1_IN0 (also ADC2_IN0, ADC3_IN0) | Port-A corner pin; easy to route to front panel |
| `CV_IN_2` | **PA1** | ADC1_IN1 (also ADC2_IN1, ADC3_IN1) | Adjacent to PA0 |
...
| Jack pin | Net |
|---|---|
| Tip | `CV_IN_1_TIP` / `CV_IN_2_TIP` — goes to R_in |
...
1.413 V V_BIAS node, because loading it will shift both channels'
zero point.
...
|---|---|---|---|
| OPA1642AIDR dual op-amp | U11 | TI OPA1642AIDR / **C67640** | Reuse BOM line (already on board as U7/U10) |
| 100 kΩ 1 % 0805 ×2 | R_in1, R_in2 | Vishay CRCW0805100KFKEA / LCSC TODO | **Add** |
| 33.0 kΩ 1 % 0805 ×2 | R_f1, R_f2 | Vishay CRCW080533K0FKEA / LCSC TODO | **Add** |
| 1.0 kΩ 1 % 0805 ×2 | R_ao1, R_ao2 | existing 1 k line | Reuse |
| 100 nF X7R 0805 ×2 | C_ao1, C_ao2 | existing 100 nF line | Reuse |
| BAT54S dual Schottky ×2 | D_clamp1, D_clamp2 | BAT54S / **C7465** | **Add (×2)** |
| 10.2 kΩ 0.1 % 0805 | R_b1 | Panasonic ERA-6AEB102V2 / LCSC TODO | **Add** |
| 10.0 kΩ 0.1 % 0805 | R_b2 | Panasonic ERA-6AEB103V / LCSC TODO | **Add** |
| 10 nF 0805 X7R | C_b | existing 10 nF line | Reuse |
| 100 nF 0805 ×2 | C_U11_vp, C_U11_vn | existing 100 nF line | Reuse |
